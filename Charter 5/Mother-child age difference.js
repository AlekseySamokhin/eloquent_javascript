...
let ANCESTRY_FILE = JSON.stringify([
  ...
  const difference = ancestry.filter((person) => byName[person.mother] != null).map((person) => person.born - byName[person.mother].born);
...
for(let i = 0; i < ancestry2.length; i++) {
    let mother = byName[ancestry2[i].mother];
